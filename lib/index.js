...
export default class PhoneInput extends Component {
  ...
  constructor(props, context) {
    ...
    this.lastNumber = countryData ? `+${countryData.dialCode}` : ""

    this.state = {
      iso2: initialCountry,
      disabled,
      formattedNumber: this.lastNumber,
      value: null
    }
  }
  ...
  onChangePhoneNumber(number) {
    if ((number.length === 1 && number[0] === "+") || number.length === 0) {
      number = this.lastNumber
    }
    else {
      if (this.changedCountry && number.length < this.lastNumber.length) {
        number = this.lastNumber
        this.changedCountry = false
      } else {
        const countryCode = this.getCountryCode()
        if (countryCode && number.length > countryCode.length)
          this.lastNumber = number
        else number = this.lastNumber
      }
    }
  ...
  selectCountry(iso2) {
    if (this.state.iso2 !== iso2) {
      ...
      this.lastNumber = `+${countryData.dialCode}`
